...
def connect():
    ...
    """
    try:
        db = MySQLdb.connect(HOST_NAME, USER_NAME, PASSWORD, DATABASE_NAME, use_unicode=True)
        return db
    except:
...
def insert_link_into_external_link(link, found_date, anchor, link_type):
    ...
    try:
        ...
        db.commit()
        cursor.execute(sql_string, (link.encode('utf8'), found_date, str(anchor.encode('utf8')), link_type))
        db.commit()
...
def external_link_already_exists(external_link):
    ...
    try:
        ...
        cursor.execute(sql_query, {"external_link" : external_link.encode('utf8')})
        tmp = cursor.fetchall()
...
def update_domain_link_url_db(domain_link_url, domain_url_old):
    db = connect()
    cursor = db.cursor()
    sql_statement1 = """
    ...
    sql_statement3 = """
                    ...
                    """
    try:
        cursor.execute(sql_statement1, (domain_link_url, domain_url_old))
        cursor.execute(sql_statement2, (domain_link_url, domain_url_old))
        cursor.execute(sql_statement3, (domain_link_url, domain_url_old))
        db.commit()
    except:
        cursor.close()
        db.close()
...
def get_notification_emails_db(domain):
    db = connect()
    cursor = db.cursor()
    sql_statement = """
...
def save_problem_db(external_id, assignment_id, detection_date, problem, recipient, fixed):
    db = connect()
    cursor = db.cursor()
    sql_statement = """
    ...
    VALUES (%s, %s, %s, %s, %s, %s)
                    """
    try:
        cursor.execute(sql_statement, (external_id, assignment_id, detection_date, problem, recipient, fixed))
        db.commit()
    except:
        cursor.close()
        db.close()
...
def select_external_by_id(external_id):
    db = connect()
    cursor = db.cursor()
    sql_statement = """
    SELECT external_links.link_url FROM external_links WHERE link_id = %(external_id)s
                    """
    try:
        cursor.execute(sql_statement, {"external_id":external_id})
        result = cursor.fetchone()
    except:
        cursor.close()
        db.close()
